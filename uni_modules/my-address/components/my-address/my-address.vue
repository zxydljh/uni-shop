		...
		methods: {
			...
			async chooseAddress() {
				try {
					...
					// 判断用户 是否授权 访问地址
					if (succ.errMsg === 'chooseAddress:fail auth deny' || succ.errMsg ===
			...
			async reAuth() {
				...
				if (confirmResult.cancel) return this.$showMsg('您取消了地址授权！');
				...
				if (confirmResult.confirm)
					return uni.openSetting({
						...
						success: settingResult => {
							...
							if (settingResult.authSetting['scope.address']) return this.$showMsg(
								'授权成功！请选择地址');
